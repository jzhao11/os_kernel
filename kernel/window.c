...
WORD default_color = 0x0F00;

void poke_screen(int x, int y, WORD ch) {
	poke_w(SCREEN_BASEADDR + y * SCREEN_WIDTH * 2 + x * 2, ch);
}
...
void scroll_window(WINDOW* wnd) {
	...
	for (int x = 0; x < wnd->width; ++x) {
		for (int y = 0; y < wnd->height - 1; ++y) {
			// copy line (y + 1) to line y
			WORD ch = peek_screen(wnd->x + x, wnd->y + y + 1);
			poke_screen(wnd->x + x, wnd->y + y, ch);
		}
	}

	// clear last line (height - 1)
	for (int x = 0; x < wnd->width; ++x) {
		poke_screen(wnd->x + x, wnd->y + wnd->height - 1, '\0');
	}
...
void output_char(WINDOW* wnd, unsigned char c) {
	...
	switch (c) {
		...
		case '\b':
			...
			break;
		default:
			poke_screen(wnd->x + wnd->cursor_x, wnd->y + wnd->cursor_y, c | default_color);
...
char* printnum(char *b, unsigned int u, int base, BOOL negflag, int length, BOOL ladjust, char padc, BOOL upcase) {
